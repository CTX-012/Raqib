...
## 2026-07-31 — classifier: recognize Gazebo + rviz2 as governable robotics workloads
- Ratified by operator (see prior investigation turn): Gazebo → `WorkloadCategory::ROS2` (no new category, no CAR); naming → new `friendly_process_name` field on ClassificationResult (D107-promote pattern); rviz2 → REVERSE the v1.0.2 Inspector #5 tooling guard so it becomes governable.
- `ClassificationResult` gains `friendly_process_name: Option<String>` + `ai_with_friendly_name(...)` constructor. Confirmed NOT in `ux_contract` — no CAR. All 3 existing constructors (`ai`, `ai_with_model`, `not_ai`) updated to set `None`.
- `runtime.rs` promotes `friendly_process_name` onto `AnnotatedProcess.name` at annotation time. Mirror of D107's `promote_sha_blob_hints` shape. `None` (the common case) preserves the raw comm.
- Gazebo detection — 2 phrase markers in `CMDLINE_KEYWORDS_WITH_FRIENDLY`: `"ign gazebo"` (Fortress) + `"gz sim"` (Harmonic). 2 name entries in `NAME_KEYWORDS_WITH_FRIENDLY`: `"gzserver"` / `"gzclient"` (classic Gazebo). All 4 emit friendly name `"Gazebo"`. Two-word phrase markers ensure a user script named `gazebo_helper.py` does NOT false-match.
- rviz2 — removed from `ROS2_TOOLING_NAMES` at `ros2.rs:197`. The library signal (`librcl.so` in `/proc/<pid>/maps`) now catches it as `WorkloadCategory::ROS2`. Other tooling names (rqt*, colcon, ament_*) kept — reversal is targeted.
- Tripwire converted (not deleted) per CLAUDE.md build-loop step 5: `rviz2_does_not_classify_as_ros2` → `rviz2_now_classifies_as_ros2_via_library_signal` + new `rviz2_no_longer_in_tooling_blacklist` guard pinning the reversal.
- Tests: 1259 → 1267 (+8): 5 positive Gazebo classification tests + 2 NEGATIVE precision guards (`plain_ruby_script_named_gazebo_helper` + `plain_ruby_process` — both MUST NOT match Gazebo) + 2 rviz2 tests (converted tripwire + tooling-blacklist guard).
- **GOVERNANCE FLAG**: Gazebo + rviz2 are now KILL-ELIGIBLE when armed (`[governor] auto_actuate = true` + `[policy] default_ai_action = "Kill"`) AND not in `[policy] allowlist`. Zero governor logic touched — `git diff HEAD src/governor/` = 0 lines. Category-feeds-existing-policy-check (D80 gate). The safety-verdict PENDING block still applies: an operator arming for a live-fire test on a host running Gazebo should add `"ruby"` + `"rviz2"` (their actual comm strings) to `[policy] allowlist` if they DON'T want them killable — even though the display shows "Gazebo", the policy check is against the comm-derived process name.
- Clippy clean. Browser gate 269/0 unchanged.
